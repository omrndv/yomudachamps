...
class IPaymuCallbackController extends Controller
{
    public function handle(Request $request)
    {
        ...
        if (isset($data['is_escrow'])) {
            $data['is_escrow'] = ($data['is_escrow'] === 'true' || $data['is_escrow'] === true || $data['is_escrow'] === 1 || $data['is_escrow'] === '1') ? true : false;
        } else {
        ...
        Log::info('IPaymu Callback JSON body for signature: ' . $jsonBody);
        ...
        // Simpan status iPaymu ke kolom status_tripay untuk konsistensi
