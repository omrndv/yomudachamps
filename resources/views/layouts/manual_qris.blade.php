        ...
        <aside id="desktop-sidebar" class="sidebar-transition w-64 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 flex flex-col shrink-0 border-r border-slate-200/80 dark:border-slate-800/80 h-full hidden xl:flex relative">
            ...
            <div class="h-24 flex items-center px-8 gap-3 shrink-0 border-b border-slate-100 dark:border-slate-800/60">
                ...
                <div class="sidebar-brand-text">
                    <h1 class="text-md font-extrabold tracking-tight leading-none text-slate-900 dark:text-white">Manual QRIS</h1>
            ...
            <nav class="flex-1 px-4 py-6 space-y-6 overflow-y-auto custom-scroll">
                <div>
                    <span class="sidebar-brand-text block text-[10px] font-bold text-slate-400 uppercase tracking-widest px-4 mb-3">Menu</span>
                    <div class="space-y-1.5">
                        ...
                        <a href="{{ route('qris.verify-payments') }}" class="w-full flex items-center gap-3.5 px-4 py-3 text-sm font-semibold rounded-2xl transition-all text-left {{ request()->routeIs('qris.verify-payments') ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:text-blue-600 hover:bg-slate-50 dark:text-slate-400 dark:hover:text-white dark:hover:bg-slate-800' }}">
                            <i data-lucide="phone" class="w-5 h-5"></i>
                            <span class="sidebar-brand-text">Verifikator Mobile</span>
                        </a>
                    </div>
                </div>
            </nav>
            ...
            <div class="p-4 border-t border-slate-100 dark:border-slate-800 shrink-0">
                ...
                <a href="{{ route('admin.logout') }}" class="w-full flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-2xl text-red-500 hover:text-red-650 hover:bg-red-50 dark:hover:bg-red-500/10 transition-all text-left font-bold">
            ...
            <div class="fixed inset-y-0 left-0 w-64 bg-white dark:bg-slate-900 text-slate-750 dark:text-white flex flex-col border-r border-slate-200 dark:border-slate-800 transform -translate-x-full transition-transform duration-300 ease-in-out">
                <div class="h-24 bg-gradient-to-r from-blue-600 to-blue-700 flex items-center justify-between px-6 shrink-0 text-white">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-sm font-black tracking-wider leading-none uppercase">Manual QRIS</h1>
                            <span class="text-[9px] text-blue-250 font-bold tracking-widest uppercase">Dashboard</span>
                        </div>
                    </div>
                ...
                <nav class="flex-1 px-4 py-6 space-y-3 overflow-y-auto custom-scroll">
                    <a href="{{ route('admin.manual-payment') }}" class="w-full flex items-center gap-3.5 px-4 py-3 text-sm font-semibold rounded-2xl text-left {{ request()->routeIs('admin.manual-payment') ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-blue-600 hover:bg-slate-50 dark:text-slate-400 dark:hover:text-white dark:hover:bg-slate-800' }}">
                        <i data-lucide="credit-card" class="w-5 h-5"></i> Pembayaran Manual
                    </a>
                    <a href="{{ route('qris.verify-payments') }}" class="w-full flex items-center gap-3.5 px-4 py-3 text-sm font-semibold rounded-2xl text-left {{ request()->routeIs('qris.verify-payments') ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-blue-600 hover:bg-slate-50 dark:text-slate-400 dark:hover:text-white dark:hover:bg-slate-800' }}">
                        <i data-lucide="phone" class="w-5 h-5"></i> Verifikator Mobile
                    </a>
                </nav>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="relative z-[9999]">
                        ...
                        <div id="profile-dropdown" class="absolute right-0 mt-3 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl py-2 hidden z-[9999]">
                            ...
                            <a href="{{ route('admin.manual-payment') }}" class="w-full text-left px-4 py-2.5 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-2 block">
                                <i data-lucide="settings" class="w-3.5 h-3.5 text-slate-400"></i> Pengaturan Manual QRIS
                            </a>
